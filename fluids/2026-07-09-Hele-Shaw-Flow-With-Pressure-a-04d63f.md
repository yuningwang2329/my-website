...
本文研究了具有尖角几何形状的 Hele-Shaw 单元内以粘度同时取决于压力和剪切速率为特征的流体的行为。该研究扩展了之前对纯压力相关（压电粘性）和屈服应力流体进行的分析，为受限复杂流动提供了新的视角。受与设计生物医学设备和与生物医学领域相关的流动、薄膜技术、注塑成型（仅举几例）相关的实际应用的推动，这里考虑的流动配置可以突出锐边周围窄间隙几何形状中复杂流体行为的基本特征。从不可压缩广义牛顿流体的控制方程出发，采用适当的流变模型，我们推导出适合 Hele-Shaw 流动的修正流动方程。在拐角区域附近获得了特定的解。数值模拟补充了理论结果，说明了流变参数对流动行为的影响。
